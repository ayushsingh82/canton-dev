...
## Alignment with Canton Development Fund Priorities

This proposal targets two of the funding priorities listed in the [2026-2028 Strategic Roadmap](https://github.com/canton-foundation/canton-dev-fund/blob/main/2026-2028-strategic-roadmap.md#requests-for-proposals):

**RFP #14 — Wallet and dApp Integration Tooling.** The roadmap seeks proposals to "improve wallet integration tooling, reusable wallet components, signing flows, account/party management, and application-to-wallet interactions." All three pieces of this proposal — pre-approval orchestration, transfer history enrichment, and the payment request format — are wallet SDK components contributed upstream to `canton-network/wallet`, in the same spirit as prior approved grants under this RFP (Cayvox Labs PartyLayer Wallet SDK, DA Canton Network dApp SDK and tooling, DA Splice wallet kernel open source).

**RFP #13 — Payments and DeFi.** The roadmap funds "open-source tooling, reference implementations, and standards, for payments, DeFi, settlement, and liquidity workflows on Canton." The payment request URI format is proposed as a CIP-level standard for payment interoperability across wallets, and the pre-approval/reward-coupon orchestration is settlement-adjacent tooling needed by any wallet or exchange operating as its own provider party — the same category as prior examples under this RFP (Deepthi's Canton payment streams, FTP's x402 protocol integration, Srikanth's settlement/DEX reference implementation, OpenZeppelin's Canton ecosystem stack).

Because the scope sits at the intersection of both RFPs, funding this as a single proposal avoids splitting closely related SDK surface area and integration tests across two grants.
